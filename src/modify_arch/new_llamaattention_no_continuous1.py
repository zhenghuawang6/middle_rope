...
class NewLlamaAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        key_states = repeat_kv(cat_key, self.num_key_value_groups)
        value_states = repeat_kv(value_states, self.num_key_value_groups)

        #进行切分
        boost_key_states, narrow_key_states = torch.chunk(key_states, 2, dim=-1)
        ...
        #     boost_attn_weights = boost_attn_weights + causal_mask
        #     narrow_attn_weights = narrow_attn_weights + causal_mask
        ...
        dtype = key_states.dtype  # 获取输入张量的 dtype 和设备信息
        min_dtype = torch.finfo(dtype).min 
            
            # if q_len != 1:  
            #     start_tokens = 16
            #     end_tokens = 500
            #     total_token = start_tokens + end_tokens
            #     mask_mask = torch.full((q_len-10-total_token, q_len-10-total_token), fill_value=min_dtype, dtype=dtype, device=device)
            #     triu_mask_mask = torch.triu(mask_mask, diagonal=1)
            #     causal_mask[:, :, total_token:q_len-10, start_tokens:start_tokens+(q_len-10-total_token)] = torch.where(triu_mask_mask.bool(), causal_mask[:, :, total_token:q_len-10, start_tokens:start_tokens+(q_len-10-total_token)] , mask_mask) # replace the group attention with neighbor attention within the neighbor window.         
        if attention_mask is not None:  # no matter the length, we just slice it
            causal_mask = attention_mask[:, :, :, : kv_seq_len]
            if q_len != 1:  
                start_tokens = 10
                end_tokens = 800
                total_token = start_tokens + end_tokens
                mask_mask = torch.full((q_len-10-total_token, q_len-10-total_token), fill_value=min_dtype, dtype=dtype,device=key_states.device)
                triu_mask_mask = torch.triu(mask_mask, diagonal=1)
                causal_mask[:, :, total_token:q_len-10, start_tokens:start_tokens+(q_len-10-total_token)] = torch.where(triu_mask_mask.bool(), causal_mask[:, :, total_token:q_len-10, start_tokens:start_tokens+(q_len-10-total_token)] , mask_mask) # replace the group attention with neighbor attention within the neighbor window.         
